...
There is one step between a huge raw text file and a heatmap, but there is an enormous one between a heatmap and an understandable heatmap (e.g. by filtering or grouping the most important data points).

Approach
--------

In the beginning of this project you adjust BioJS components from the [registry][registry] to [Galaxy datatypes][datatypes].

With the help of [biojs2galaxy](https://github.com/biojs/biojs2galaxy) you will be able to convert BioJS components to Galaxy visualization plugins. At the moment most BioJS components are constructed with a settings object, hence you should provide a input mask for the user to adjust those settings and update the components on changes. 

Using this gained experience you will be add BioJS components to [Galaxy Charts Plugin](https://wiki.galaxyproject.org/Learn/Visualization/Charts) - an component builder provided by the Galaxy project.
